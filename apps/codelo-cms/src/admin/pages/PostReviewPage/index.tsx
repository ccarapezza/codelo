  ...
  return (
    ...
    >
      <Flex direction={isMobile ? "column" : "row"} gap={isMobile ? 3 : 4} alignItems="stretch">
        ...
        <Flex direction="column" gap={2} style={{ flexShrink: 0, width: isMobile ? "100%" : 170 }}>
          ...
          {/* Editar: abre el editor unificado con la nota cargada. Reemplaza al
              Content Manager como vía de edición del día a día. */}
          <Button
            variant="secondary"
            size="S"
            fullWidth
...
export default function PostReviewPage() {
  ...
  const handleToggle = async (note: Note, action: "publish" | "unpublish") => {
    ...
    try {
      await post(action === "publish" ? PUBLISH_API : UNPUBLISH_API, {
