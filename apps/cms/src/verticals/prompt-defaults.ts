...
//
// Es una COSTURA. El motor (lib/prompt-defaults.ts) define la estructura y
// unos valores neutros; acá se reemplazan por los del proyecto, y el admin
// puede volver a pisarlos desde la pantalla de Prompts IA.
//
// Es el archivo que hay que reescribir entero para apuntar el motor a otro
// tema. Las reglas duras del dominio —la cláusula del estatuto, el no fomento
// de consumo, las imágenes sin consumo ni caras— viven acá y no se relajan.
...
export const promptDefaults = {
  ...
  domainDescription: "the information portal of an Argentine non-profit civil association (asociación civil) whose statutory objects are the research and study of cannabis cultivation and its derivatives under Argentina's Ley 27.350 framework, the ethnobotanical study of plants (Plantae) and fungi (Fungi) — with emphasis on Cannabis (Cannabaceae), including hemp for agro-industrial and food use — human rights (right to health, food sovereignty), harm-reduction approaches to problematic substance use, and environmental preservation; it also covers home growing, the REPROCANN registry, cannabis law news in Argentina, and the association's community activities. It never promotes the consumption of any substance, licit or not.",
  writingLanguage: "Spanish",
  fabricationProneFacts: "legal requirements, REPROCANN rules or deadlines, medical or dosage claims, event dates, names of officials, or court rulings",
  analysisModeFraming: "clearly framed as opinion or analysis (e.g. 'Análisis:', 'Lo que sabemos de…'). Never state a recent event as fact, and never present legal or medical interpretation as certainty.",
